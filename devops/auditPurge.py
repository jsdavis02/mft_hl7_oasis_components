...
select_num_records = "SELECT value FROM app_settings WHERE name = 'num_records_purge' AND env = '"+env+"'"
cursor = cnxn.cursor()
cursor.execute(select_num_records)
row = cursor.fetchone()
num_records = row[0]

select_days_old = "SELECT value FROM app_settings WHERE name = 'days_old_purge' AND env = '"+env+"'"
cursor = cnxn.cursor()
...
if args.audit_record is not None:
    ...
    select_records = "SELECT TOP "+num_records+" * FROM audit WHERE created_at <= dateadd(day, "+days+", getdate())"
...
with open('/opt/archive/oasis/logs/auditpurge.log', "a+") as log:
    log.write("\nBW Run Time: "+str(current_time)+' : Python Log Write Time: '+str(datetime.now())+' Hostname : '+str(socket.gethostname())+' : # of Records to Purge : '+str(len(audit_records))+"\n")
for record in audit_records:
    record_id = record['id']
    print('Audit Record: '+str(record_id))
...
# Check for audit records that are for mft and require data persistence, ie, save the transfer file(s)
    if record['data_format'] == 'mft' and record['MessageReference'] is not None and len(record['MessageReference']) > 0:
        ...
        if not os.path.exists(mft_archive_path):
            with open('/opt/archive/oasis/logs/auditpurge.log', "a+") as log:
                log.write("\nBW Run Time: "+str(current_time)+' : Python Log Write Time: '+str(datetime.now())+' : creating folder : '+str(mft_archive_path)+"\n")
